package com.sanalstil.pEJB.pDatabase.pOracle.pIddaa.pBetList.pVsanalstilyball;
...
public class cVsanalstilyball  extends cBaseDBPackage 
{
	public cVsanalstilyball(cOracleDB _OrcaleDb)
	{
		super(_OrcaleDb);
		LoadSubPackage();
	}
	...
	public cDataJsonResult GetMatchProgram()
	{
        ...
        try {
        	...
        	cStartup.ServerLogger.FileInfo("Called cVsanalstilyball->GetMatchProgram()");
        	
        	__Connection = OrcaleDB.DataSource.getConnection();
           	__PrepareCall = __Connection.prepareCall("begin EBAHIS.EMI_P_IDDAA_BETLIST_v2.getVsanalstilyball(?); end;");
			...
			//System.out.print("voleyball getMatchProgram dataHashList :"+dataHashList);
			JSONArray tDateList = new JSONArray();
			
			
			JSONArray tLnsArray = new JSONArray();
			 
			String[] tempFixOne = {"F.1","F.2"};
			...
			if(tLnsArray.size()>0){
		        ...
		        try 
		        {
		            __PrepareCall = __Connection.prepareCall("begin EBAHIS.EMI_P_IDDAA_BETLIST_v2.getCbsLeagueNamesVsanalstilyball(?, ?); end;");
			        __PrepareCall.registerOutParameter(1, OracleTypes.CURSOR);
				...
				{
					__Fail = true;
					cStartup.ServerLogger.ConsolError("Error..! -> com.sanalstil.pEJB.pDatabase.pOracle.pIddaa.pBetList.pVsanalstilyball.cVsanalstilyball.GetMatchProgram()");
        			cStartup.ServerLogger.FileError("Error..! -> com.sanalstil.pEJB.pDatabase.pOracle.pIddaa.pBetList.pVsanalstilyball.cVsanalstilyball.GetMatchProgram()", _Ex);
				}
			}
			...
			if (!__Fail)
			{
				cStartup.ServerLogger.FileInfo("Called cVsanalstilyball->GetMatchProgram() -> SUCCESS");
			}
			else
			{
				cStartup.ServerLogger.FileInfo("Called cVsanalstilyball->GetMatchProgram() -> Sub try catch error..!");
			}
